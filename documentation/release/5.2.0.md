...
[NOUVEAUTE]
* Changement des fiches métiers
* Ajout de back-office pour les missions et les activités
* Ajout de la notion de code fonction
* Amélioration des imports : activite, mission, compétence et fiche metier
* ...
...
ALTER TABLE structure_observateur  DROP CONSTRAINT IF EXISTS structure_observateur_structure_id_fk;
ALTER TABLE structure_contact  DROP CONSTRAINT IF EXISTS structure_contact_structure_id_fk;
ALTER TABLE entretienprofessionnel_delegue  DROP CONSTRAINT IF EXISTS entretienprofessionnel_delegue_structure_id_fk;
ALTER TABLE poste  DROP CONSTRAINT IF EXISTS poste_structure_source_id_fk;
ALTER TABLE structure DROP CONSTRAINT IF EXISTS structure_niv2_id_fk;
ALTER TABLE structure DROP CONSTRAINT IF EXISTS structure_niv2_id_ow_fk;
    
-- Changement de type
ALTER TABLE structure ALTER COLUMN id TYPE VARCHAR(1024) USING id::varchar;
ALTER TABLE structure ALTER COLUMN parent_id TYPE VARCHAR(1024) USING parent_id::varchar;
ALTER TABLE structure ALTER COLUMN niv2_id TYPE VARCHAR(1024) USING niv2_id::varchar;
ALTER TABLE structure ALTER COLUMN niv2_id_ow TYPE VARCHAR(1024) USING niv2_id_ow::varchar;
...
alter table element_competence drop column IF EXISTS  source;
...
alter table element_competence drop column IF EXISTS referentiel_id_old;
drop table IF EXISTS element_competence_referentiel cascade;
...
drop table IF EXISTS metier_reference cascade;
drop table IF EXISTS metier_referentiel cascade;
...
-- ---------------------------------------------------------------------------------------------------------------------
drop table IF EXISTS fichereferentiel_competence;
drop table IF EXISTS fichereferentiel_fiche;
...
ALTER TABLE missionprincipale_old ALTER COLUMN id DROP DEFAULT;
...
drop table IF EXISTS missionprincipale_application;
drop table IF EXISTS missionprincipale_competence;
drop table IF EXISTS missionprincipale_domaine;
...
SELECT setval(
               ...
               (SELECT COALESCE(MAX(id), 0) FROM missionprincipale) + 1,
               false
       );
------------------------------------------------------------------------------------------------------------------------
...
ALTER TABLE activite_old ALTER COLUMN id DROP DEFAULT;
alter table fichemetier_activite rename to fichemetier_activite_old;


-- elagage de tables abandonnées
drop table IF EXISTS activite_application;
drop table IF EXISTS activite_competence;
drop table IF EXISTS activite_domaine;    
...
SELECT setval(
   'activite_id_seq',
   (SELECT COALESCE(MAX(id), 0) FROM activite) + 1,
   false
);
...
alter table fichemetier drop column IF EXISTS code_fonction;
...
drop table IF EXISTS metier_metier_domaine cascade;
drop table IF EXISTS metier_domaine_famille cascade;
drop table IF EXISTS metier_domaine cascade;
drop table IF EXISTS metier_metier_correspondance cascade;
drop table IF EXISTS metier_metier_famille;
alter table fichemetier drop constraint IF EXISTS fichetype_metier__fk;
alter table fichemetier drop column IF EXISTS metier_id;
drop table IF EXISTS metier_metier;

UPDATE ficheposte_fichemetier fpfm
SET activites = (
    SELECT string_agg(ae.activite_id::text, ';')
    FROM activite_element ae
    WHERE ae.fichemetier_id = fpfm.fiche_type
);
...
drop table IF EXISTS ficheposte_fichemetier_domaine;
...
alter table ficheposte_fichemetier drop column IF EXISTS activites;
...
drop table IF EXISTS missionprincipale_old;
drop table IF EXISTS fichemetier_mission_old;
alter table missionprincipale drop column IF EXISTS old_id;
alter table missionprincipale_element drop column IF EXISTS fichemetier_id;
update missionprincipale_element set description = null where description ilike '5.1.0 %';

-- NETTOYAGE (ACTIVITE)
drop table IF EXISTS missionprincipale_activite;
drop table IF EXISTS fichemetier_activite_old;
alter table activite drop column IF EXISTS old_id;
alter table activite_element drop column IF EXISTS fichemetier_id;
```

### Avec BddAdmin

Cette version contient de changement pour recommender l'usage de bddadmin. 
...
update unicaen_parametre_parametre set valeur='true' where code='DISPLAY_ACTIVITE';
```
...
```postgresql
-- Décalage de compétences spécifiques
...
drop table IF EXISTS fichemetier_competence_specifique;
```
...
delete from unicaen_privilege_privilege where categorie_id in (select id from unicaen_privilege_categorie where code in ('ficheprofil'));
delete from unicaen_privilege_categorie where code in ('ficheprofil');
delete from unicaen_privilege_privilege where categorie_id in (select id from unicaen_privilege_categorie where code in ('domaine', 'metier'));
delete from unicaen_privilege_categorie where code in ('domaine', 'metier');
delete from unicaen_privilege_privilege where categorie_id in (select id from unicaen_privilege_categorie where code in ('mailing'));
delete from unicaen_privilege_categorie where code in ('mailing');
...
drop table IF EXISTS activite_description;
drop table IF EXISTS activite_libelle;
drop table IF EXISTS agent_element_formation;
drop table IF EXISTS entretienprofessionnel_observation_old;
drop table IF EXISTS ficheposte_expertise;
drop table IF EXISTS import_log;
drop table IF EXISTS lagaf_stagiaire;
drop table IF EXISTS source;
drop table IF EXISTS table_name;
drop table IF EXISTS ficheposte_activitedescription_retiree;

-- Clarification de libellés
update unicaen_privilege_categorie set namespace='Carriere\Provider\Privilege' where namespace='Metier\Provider\Privilege';
update unicaen_privilege_categorie set namespace='UnicaenAutoform\Provider\Privilege' where namespace='Autoform\Provider\Privilege';
update unicaen_privilege_categorie set namespace='FicheMetier\Provider\Privilege' where code='fichemetier';

update unicaen_privilege_categorie set libelle='Gestion des activités' where code='activite';
update unicaen_privilege_privilege set libelle='Afficher' where code='activite_afficher';
update unicaen_privilege_privilege set libelle='Ajouter' where code='activite_ajouter';
update unicaen_privilege_privilege set libelle='Modifier' where code='activite_modifier';
update unicaen_privilege_privilege set libelle='Accéder à l''index' where code='activite_index';
update unicaen_privilege_privilege set libelle='Historiser/Restaurer' where code='activite_historiser';
...
Mise a jour du template `FICHE_METIER` : le template a été complétement refondu et est passé en `twig`.
...
Dans la catégorie Agent on a maintenant le paramêtre `ONGLET_MISSIONS` qui permet de masquer l'onglet présentant les missions spécifiques des agents. 
Dans la catégorie Fiche métier on a le paramètre `DISPLAY_ACTIVITE` pour le nouveau bloc/module Activités.

## Évolution de la liste des privileges

Beaucoup d'ajustements ont été faits dans les privilèges. Il est nécessaire de les reparcourir.
