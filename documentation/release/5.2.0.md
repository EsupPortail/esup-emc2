...
alter table activite add codes_fiche_metier text;
alter table activite add codes_fonction text;
alter table activite add raw text;
alter table activite alter column histo_creation set default now();
alter table activite alter column histo_createur_id set default 0;
alter table activite alter column histo_modification drop not null;
alter table activite alter column histo_modificateur_id drop not null;
...
drop table activite_application cascade ;
drop table activite_competence cascade ;
drop table activite_domaine cascade ;
...
-- select DISTINCT fm.id, fm.libelle, count(m.libelle_default), max(m.libelle_default) from fichemetier fm join metier_metier m on fm.metier_id = m.id group by fm.id;
UPDATE fichemetier fm SET libelle = m.libelle_default FROM metier_metier m WHERE fm.metier_id = m.id AND fm.metier_id IS NOT NULL;
...
-- ---------------------------------------------------------------------------------------------------------------------
-- modification des missions dans les fiches metiers
-- ---------------------------------------------------------------------------------------------------------------------

alter table missionprincipale add column description text;    
alter table missionprincipale add column source_string text;
...
select fmo.fichemetier_id, me.id  from missionprincipale_element me join fichemetier_mission_old fmo on fmo.mission_id = me.description::int;

-- ---------------------------------------------------------------------------------------------------------------------
-- modification des activités dans les fiches metiers
-- ---------------------------------------------------------------------------------------------------------------------

alter table activite add column libelle varchar(1024);
alter table activite add column description text;
...
(
    ...
    activite_id           integer                 not null
        constraint activite_element_activite_id_fk references public.activite on delete cascade,
    description           text,
    position              integer   default 9999  not null,
    histo_creation        timestamp default now() not null,
    histo_createur_id     integer   default 0     not null
        constraint activite_element_unicaen_utilisateur_user_id_fk references public.unicaen_utilisateur_user,
    histo_modification    timestamp,
    histo_modificateur_id integer
        constraint activite_element_unicaen_utilisateur_user_id_fk_2 references public.unicaen_utilisateur_user,
    histo_destruction     timestamp,
    histo_destructeur_id  integer
        constraint activite_element_unicaen_utilisateur_user_id_fk_3 references public.unicaen_utilisateur_user
);

create table fichemetier_activite
(
    fichemetier_id      integer not null
        constraint fichemetier_activite_fichemetier_id_fk references public.fichemetier on delete cascade,
    activite_element_id integer not null
        references public.activite_element on delete cascade,
    constraint fichemetier_activite_pk primary key (fichemetier_id, activite_element_id)
);

insert into activite (libelle, description, referentiel_id, reference)
select mpa.libelle, fmo.fichemetier_id, fm.referentiel_id, mpa.id
from fichemetier_mission_old fmo
join missionprincipale_activite mpa on fmo.mission_id = mpa.mission_id
join fichemetier fm on fmo.fichemetier_id = fm.id;

insert into activite_element (activite_id, description, position)
select a.id, a.description, coalesce(mpa.ordre,9999) + coalesce(fmo.ordre,9999) * 10000
from activite a
join missionprincipale_activite mpa on a.reference::int = mpa.id
join fichemetier_mission_old fmo on mpa.mission_id = fmo.mission_id;

insert into fichemetier_activite (fichemetier_id, activite_element_id)
select ae.description::int, ae.id
...
alter table fichemetier add codes_emploi_type varchar(1024);
