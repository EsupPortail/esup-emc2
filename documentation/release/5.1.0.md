...
Une évolution de l'envoi de mail de symfony force l'ajout d'une clef de configuration pour la bibliothèque `unicaen/mail`.
Dans le fichier `config/autoload/unicaen-mail.local.php` il faut ajouter la clef `tls` si vous utilisez un envoi standard :
```php
 'unicaen-mail' => [
        // ...
        'transport_options' => [
...
### Modification de la structure [Sans BDDADMIN]

```postgresql
-- Tables associées à la bibliothèque unicaen/contact
...
-- Ajout de la table de jointure entre les contacts et les structures
...
create table fichemetier_tendance_type
(
    id                    serial
        constraint fichemetier_tendance_type_pk
            primary key,
    code                  varchar(256)            not null
        constraint fichemetier_tendance_type_pk_2
            unique,
    ...
    histo_creation        timestamp default now() not null,
    histo_createur_id     integer   default 0     not null
        constraint fichemetier_tendance_type_unicaen_utilisateur_user_id_fk
            references unicaen_utilisateur_user,
    histo_modification    timestamp,
    histo_modificateur_id integer
        constraint fichemetier_tendance_type_unicaen_utilisateur_user_id_fk_2
            references unicaen_utilisateur_user,
    histo_destruction     timestamp,
    histo_destructeur_id  integer
        constraint fichemetier_tendance_type_unicaen_utilisateur_user_id_fk_3
            references unicaen_utilisateur_user,
    ordre                 integer   default 9999  not null
);

create table fichemetier_tendance_element
(
    id                    serial
        constraint fichemetier_tendance_element_pk
            primary key,
    fichemetier_id        integer                 not null
        constraint fichemetier_tendance_element_fichemetier_id_fk
            references fichemetier
            on delete cascade,
    tendancetype_id       integer                 not null
        constraint fme_fichemetier_tendance_type_id_fk
            references fichemetier_tendance_type
            on delete cascade,
    texte                 text,
    histo_creation        timestamp default now() not null,
    histo_createur_id     integer   default 0     not null
        constraint fichemetier_tendance_element_unicaen_utilisateur_user_id_fk
            references unicaen_utilisateur_user,
    histo_modification    timestamp,
    histo_modificateur_id integer
        constraint fichemetier_tendance_element_unicaen_utilisateur_user_id_fk_2
            references unicaen_utilisateur_user,
    histo_destruction     timestamp,
    histo_destructeur_id  integer
        constraint fichemetier_tendance_element_unicaen_utilisateur_user_id_fk_3
            references unicaen_utilisateur_user
);

-- partie durée de l'entretien professionnel pour les rendez-vous ICS
alter table entretienprofessionnel add column duree_estimee float;

-- Modification associé à l'import des fiches métiers
...
### Modification de la structure [Avec BDDADMIN]
...
Insertion de nouveaux privilèges
```postgresql
-- Chaines hiérarchiques
...
-- BackOffice des tendances (fiche métier)
INSERT INTO unicaen_privilege_categorie (code, libelle, ordre, namespace) VALUES 
('tendancetype', 'Gestion des types de tendance', 200, 'FicheMetier\Provider\Privilege');
INSERT INTO unicaen_privilege_privilege(CATEGORIE_ID, CODE, LIBELLE, ORDRE)
WITH d(code, lib, ordre) AS (
...
-- Privilèges pour unicaen/contact
...
-- Affichage de la configuration du mailer
INSERT INTO unicaen_privilege_privilege(CATEGORIE_ID, CODE, LIBELLE, ORDRE)
WITH d(code, lib, ordre) AS (
...
```postgresql
-- Affichage des emploi-types avec agent
...
-- Délai d'émission des observations
INSERT INTO unicaen_parametre_parametre(CATEGORIE_ID, CODE, LIBELLE, DESCRIPTION, VALEURS_POSSIBLES, ORDRE)
WITH d(CODE, LIBELLE, DESCRIPTION, VALEURS_POSSIBLES, ORDRE) AS (
...
-- Affichage du bloc tendance sur les fiche métier
INSERT INTO unicaen_parametre_parametre(CATEGORIE_ID, CODE, LIBELLE, DESCRIPTION, VALEURS_POSSIBLES, ORDRE)
WITH d(CODE, LIBELLE, DESCRIPTION, VALEURS_POSSIBLES, ORDRE) AS (
    SELECT 'DISPLAY_TENDANCE', 'Affichage du bloc "Tendances d''évolution"', null, 'Boolean', 80
)
SELECT cp.id, d.CODE, d.LIBELLE, d.DESCRIPTION, d.VALEURS_POSSIBLES,  d.ORDRE
FROM d
JOIN unicaen_parametre_categorie cp ON cp.CODE = 'FICHE_METIER';

-- Affichage du bloc contact sur la fiche de la structure
INSERT INTO unicaen_parametre_parametre(CATEGORIE_ID, CODE, LIBELLE, DESCRIPTION, VALEURS_POSSIBLES, ORDRE)
WITH d(CODE, LIBELLE, DESCRIPTION, VALEURS_POSSIBLES, ORDRE) AS (
...
```postgresql
update unicaen_parametre_parametre set valeur='5.1.0' where code='VERSION';
update unicaen_parametre_parametre set valeur='21/10/2025' where code='RELEASE_DATE';
```

## Évolution des librairies

Ajout d'unicaen/contact
Montée en version `unicaen/aide` 
Montée en version `unicaen/indicateur` (6.4.0) permettant la gestion des périmètres `_ALL`
Fixage de la version de `unicaen/privilege` à 6.2.1 pour éviter les effet de bord provoqué par la version 6.3.0
...
* `FICHE_METIER|DISPLAY_TENDANCE` : affichage du bloc tendance

## Évolution de la liste des privileges

Suite à l'évolution des bibliothèques de nouveaux privilèges ont été introduits :
* `mail_afficher_config` pour permettre l'affichage de la configuration du mailer (recommandation ne le donner qu'aux administrateur technique)
* `parametre_afficher_masquer` pour permettre l'afficahge des paramètres masqués (recommandation ne le donner qu'aux administrateurs)

La biliothèque de gestion de contacts vient avec son lot de privilèges
* la catégorie `contacttype` pour l'accès à la déclaration des différents types de contact (en backoffice)
* la catégorie `contact` pour la gestion des contacts eux-mêmes (par exemple, sur les descriptions des structures).

L'ajout des tendances sur les fiches métiers introduit un lot de paramètres pour la gestion du backoffice
* la catégorie `tendancetype`

Afin de faciliter l'investigation sur les chaînes hiérarchiques un privilège a été ajouter pour contrôler l'affichage de l'historisuqe des chaines hiérarchiques sur la partie agent
