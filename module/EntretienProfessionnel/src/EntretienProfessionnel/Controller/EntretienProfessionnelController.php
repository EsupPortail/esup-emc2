...
class EntretienProfessionnelController extends AbstractActionController
{
    ...
    /** Pour le moment la recherche se fait sur la dénomination de l'agent */
    public function rechercherAction(): JsonModel
    {
        if (($term = $this->params()->fromQuery('term'))) {
            $entretiens = $this->getEntretienProfessionnelService()->findEntretienByTerm($term);
            $result = $this->getEntretienProfessionnelService()->formatEntretienJSON($entretiens);
            return new JsonModel($result);
        }
    }
    ...
    public function retourListeAction(): Response
    {
        ...
        switch($role->getRoleId()) {
            ...
            case  Agent::ROLE_AUTORITE :
                ...
                return $this->redirect()->toRoute('entretien-professionnel/campagne/autorite', ['agent' => $agent?->getId(), 'campagne' => $campagne?->getId()],[],true);
