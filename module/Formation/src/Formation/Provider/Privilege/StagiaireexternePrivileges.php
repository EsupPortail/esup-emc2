<?php

namespace Formation\Provider\Privilege;

use UnicaenPrivilege\Provider\Privilege\Privileges;

class StagiaireexternePrivileges extends Privileges
{
    const STAGIAIREEXTERNE_INDEX = 'stagiaireexterne-stagiaireexterne_index';
    const STAGIAIREEXTERNE_AFFICHER = 'stagiaireexterne-stagiaireexterne_afficher';
    const STAGIAIREEXTERNE_AJOUTER = 'stagiaireexterne-stagiaireexterne_ajouter';
    const STAGIAIREEXTERNE_MODIFIER = 'stagiaireexterne-stagiaireexterne_modifier';
    const STAGIAIREEXTERNE_HISTORISER = 'stagiaireexterne-stagiaireexterne_historiser';
    const STAGIAIREEXTERNE_SUPPRIMER = 'stagiaireexterne-stagiaireexterne_supprimer';
}
