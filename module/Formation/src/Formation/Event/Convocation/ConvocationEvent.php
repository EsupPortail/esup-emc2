...
class ConvocationEvent extends  EvenementService
{
    ...
    public function traiter(Evenement $evenement) : string
    {
        ...
        try {
            ...
            if ($session AND $session->estNonHistorise()
                ...
                $log = "Session #".$session->getId()." ".$session->getInstanceLibelle()." : Envoi des convocations";
            }
    ...
    public function updateEvent(Seance $seance): void
    {
        ...
        if (!$dateTraitement instanceof DateTime) {
            throw new RuntimeException("La date de traitement de l'evenement [".EvenementProvider::CONVOCATION."] n'a pas pu être déterminée.");
        }
